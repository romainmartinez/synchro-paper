...
def checkout_existing_versions(args):
    ...
    print(
        "Attempting checkout with the following command:", " ".join(command), sep="\n"
    )
...
def create_version(args):
    ...
    for src, dst in renamer.items():
        ...
        if not src_path.exists():
            continue
        shutil.copy2(src=src_path, dst=args.version_directory.joinpath(dst))
...
def ots_upgrade(args):
    ...
    for ots_path in ots_paths:
        ...
        if process.returncode != 0:
            print(
                f"OpenTimestamp upgrade command returned nonzero code ({process.returncode})."
            )
        if not process.stderr.strip() == "Success! Timestamp complete":
            print(f">>> {' '.join(map(str, process.args))}\n" f"{process.stderr}")
